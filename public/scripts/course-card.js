import {
    addTeacher, changeCourseDetails, changeCourseRequirementsAndAnnotations,
    changeCourseStatus,
    createNotification,
    editStatusStudent,
...
export async function setupCourseCardPage(params) {
    ...
    await setupChangeCourseDetailsModal(courseDetails, roles, myEmail)
...
function setupMainCourseDetails(courseDetails) {
    ...
    document.getElementById(`course-name`).value = courseDetails['name'];
    document.getElementById(`course-year`).value = courseDetails['startYear'];
    document.getElementById(`course-semester-spring`).checked = courseDetails['semester'] === 'Spring';
    document.getElementById(`course-semester`).checked = courseDetails['semester'] !== 'Spring';
    document.getElementById(`course-students-count`).value = courseDetails['maximumStudentsCount'];
}
...
function setupCourseStatus(status) {
    document.getElementById(`status`).innerHTML = translate(status).status;
    document.getElementById(`status`).style.color = translate(status).color;
...
function setupStudents(courseDetails, roles) {
    ...
    courseDetails['students'].forEach((student) => {
        ...
        const forButtons = studentNode.getElementsByClassName('for-buttons-results')[0];
        const middleTermResultContainerNode = studentNode.getElementsByClassName("midterm-result-container");
        const finalResultLabelContainerNode = studentNode.getElementsByClassName("final-result-container");
        ...
        if (student ['status'] === 'Accepted' && student['midtermResult'] && student['finalResult']) {
            middleTermResultContainerNode[0].style.display = null
            finalResultLabelContainerNode[0].style.display = null
            middleTermResultContainerNode[1].style.display = null
            finalResultLabelContainerNode[1].style.display = null
        ...
        if (student['status'] === 'InQueue') {
            ...
            forButtons.appendChild(studentStatusChangeNode)
        }
...
export function translate(enumValue) {
    switch (enumValue) {
        case 'InQueue':
            return 'В очереди'
        case 'NotDefined':
        ...
        case 'Declined':
            return 'Отклонен'
        case 'Created':
            return {status: 'Создан', color: '#9a9a9a'}
        case 'OpenForAssigning':
            return {status: 'Открыт для записи', color: '#009e0f'}
        case 'Started':
            return {status: 'В процессе обучения', color: '#0070ed'}
        case 'Finished':
            return {status: 'Закрыт', color: '#ed1800'}
        case 'Autumn':
            return 'Осенний'
        case 'Spring':
            return 'Весенний'
